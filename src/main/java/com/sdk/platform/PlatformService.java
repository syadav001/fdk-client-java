...
public class ApplicationClient {
    ...
    public PlatformModels.SuccessResponse upsertAppProductMeta(String itemId ) throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.SuccessResponse> response = catalogApiList.upsertAppProductMeta(this.companyId , this.applicationId , itemId ).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
