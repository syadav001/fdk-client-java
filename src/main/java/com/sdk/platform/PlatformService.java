...
public static class CatalogService {
    ...
    public PlatformModels.SuccessResponse1 upsertInventory(Double itemId , String sellerIdentifier ,PlatformModels.InventoryRequestSchemaV2 body) throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.SuccessResponse1> response = catalogApiList.upsertInventory(this.companyId , itemId  , sellerIdentifier  , body).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
    ...
    public PlatformModels.SuccessResponse1 upsertBulkInventory(PlatformModels.BulkInventoryRequestSchemaV2 body) throws IOException {
        if (this.platformConfig.getPlatformOauthClient().isAccessTokenValid()) {
            Response<PlatformModels.SuccessResponse1> response = catalogApiList.upsertBulkInventory(this.companyId , body).execute();
            if (!response.isSuccessful()) {
                    throw new IOException(response.errorBody() != null
                            ? response.errorBody().string() : Fields.UNKNOWN_ERROR);
                }
